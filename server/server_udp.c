...
int main(int argc, char *argv[])
{
     ...
     // Code for RRTs of UDP
     /*	
     int seq = 1;	
     while ( 1 ){
	 
         struct msg_t message;
	 state = recvfrom( sockfd, &message, sizeof( message ), MSG_WAITALL, ( struct sockaddr *) &cli_addr, &clientlen );
         if ( state < 0 ){ 
             perror( "ERROR reading from socket\n" );
	     exit( 0 );
         }
	 make_msg( &message, 4, seq, 10, 0, "" );
         sendto( sockfd, &message, sizeof( message ), 0, ( struct sockaddr * ) &cli_addr, clientlen );
 	 seq ++; 
     }	     		
     */
     ...
     if ( message.msg_type == 1 ){
 	 ...
 	 // Check if file open succeed. If not, sent a MSG_TYPE_GET_ERR message back to the client
	     ...
	     sendto( sockfd, &openfailmsg, sizeof( openfailmsg ), 0, ( struct sockaddr * ) &cli_addr, clientlen );
         }
	 int indicator = 1;
	 int seq = 1; 
	 ...
	 int max_seq = ( ( size - 1 ) / BUF_SZ ) + 1;
	     ...
	     sendto( sockfd, &res, sizeof( res ), 0, ( struct sockaddr *) &cli_addr, clientlen ); 
	     
	     // Receive message  
	     struct msg_t recvmsg;	
             int r = recvfrom( sockfd, &recvmsg, sizeof( recvmsg ), MSG_WAITALL, ( struct sockaddr *) &cli_addr, &clientlen );
             if (r < 0) {
	 ...
	 struct msg_t finmsg;	
         recvfrom( sockfd, &finmsg, sizeof( finmsg ), MSG_WAITALL, ( struct sockaddr *) &cli_addr, &clientlen );
